...
TABLE_NAME = "ffarticles"

def load_config(config_path):
    with open(config_path, 'r') as file:
        return yaml.safe_load(file)

def load_text_file(file_path, url, title):
...
def process_articles(config):
    ...
    for article_key, article_info in config['web_articles'].items():
        file_path = article_info['output_path']
        url = article_info['url']
        title = article_info['title']
        
        if os.path.exists(file_path):
            try:
                txt_docs = load_text_file(file_path, url, title)
...
def main():
    config_path = 'bot//article_config.yml'  
